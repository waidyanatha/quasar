...
#d            g_clusters.add_node(j1, attr_dict={'attractor':density_attractors[j1], 'radius':radii[j1],
#d                                'density':density[j1]})
#d            g_clusters.add_node(j1, attractor=density_attractors[j1],
#d                                radius=radii[j1],
#d                                density=density[j1])
            ''' @waidyanatha applied this fix recommended here:
                https://stackoverflow.com/questions/64841083/adding-a-node-with-attribute-in-a-networkx-graph
            '''
            g_clusters.add_nodes_from([(j1, {'attractor':density_attractors[j1],
                                        'radius':radii[j1],
                                        'density':density[j1]})])
...
#d        clusters = list(nx.connected_component_subgraphs(g_clusters))
            ''' @waidyanatha applied this fix recommended here:
...
#d        clusters = nx.connected_components(g_clusters)
        num_clusters = 0
...
#d            max_instance = max(clust, key=lambda x: clust.nodes[x]['density'])
#d            max_density = clust.nodes[max_instance]['density']
#d            max_centroid = clust.nodes[max_instance]['attractor']
            ...
            complete = False
#d            c_size = len(clust.nodes())
            c_size = len(g_clusters.nodes())
#d            if clust.number_of_edges() == (c_size*(c_size-1))/2.:
            if g_clusters.number_of_edges() == (c_size*(c_size-1))/2.:
                complete = True

            #populate cluster_info dict
#d            cluster_info[num_clusters] = {'instances': clust.nodes(),
            cluster_info[num_clusters] = {'instances': g_clusters.nodes(),
                                        'size': c_size,
...
#d                labels[clust.nodes()]=num_clusters
                labels[g_clusters.nodes()]=num_clusters
            num_clusters += 1
